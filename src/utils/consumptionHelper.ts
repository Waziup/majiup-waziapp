...
export async function getConsumption(
  ...
  const response = await axios.get(
    `${import.meta.env.VITE_BACKEND_URL}/tanks/${deviceId}/tank-info`,
    {
      headers: {
    ...
    for (const property in obj) {
      ...
      if (obj.hasOwnProperty(property) && Array.isArray(obj[property])) {
        if (index < obj[property].length) {
          ...
          newObj.liters =
            (obj["waterLevel"][obj["waterLevel"].length - 1].value /
              tankHeight) *
            tankCapacity;
        } else {
...
export const postNewNotificationMessage = async (
  ...
  if (tank) {
    ...
    const response = await axios.post(
      `${import.meta.env.VITE_BACKEND_URL}/tanks/${deviceId}/meta`,
      {
        ...tank.meta,
        notifications: {
